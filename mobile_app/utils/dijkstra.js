// Source: https://gist.github.com/Prottoy2938/66849e04b0bac459606059f5f9f3aa1a
...
export class WeightedGraph {
  ...
  constructGraph(floorPlanGraph, distancePerPixel) {
    floorPlanGraph.forEach((node) => this.addVertex(node.id.toString()));
    floorPlanGraph.forEach((node) => {
      node.edges.forEach((edge) => {
        ...
        this.addEdge(
          ...
          distancePerPixel *
            Math.sqrt(
              (node.imageX - edgeX) * (node.imageX - edgeX) +
                (node.imageY - edgeY) * (node.imageY - edgeY)
            )
        );
      });
    });
